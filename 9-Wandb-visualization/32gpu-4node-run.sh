...
#SBATCH --gpus-per-node=8            # Number of GPUs per node (32 total)
...
#SBATCH --time=30:00               # time limit for 32 GPU run
#SBATCH --output=slurm/32gpu-4node-run.sh-slurm-%j.out      # Tab-completion friendly output filename
...
srun singularity exec -B ../resources/visualtransformer-env.sqsh:/user-software:image-src=/ $CONTAINER bash -c "export RANK=\$SLURM_PROCID && export LOCAL_RANK=\$SLURM_LOCALID && cd ../resources && python ../9-Wandb-visualization/scripts/32gpu-4node-wandb.py"
