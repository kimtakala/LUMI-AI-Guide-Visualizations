...
#SBATCH --output=slurm/24gpu-3node-run.sh-slurm-%j.out      # Tab-completion friendly output filename
...
srun singularity exec -B ../resources/visualtransformer-env.sqsh:/user-software:image-src=/ $CONTAINER bash -c "export RANK=\$SLURM_PROCID && export LOCAL_RANK=\$SLURM_LOCALID && cd ../resources && python ../9-Wandb-visualization/scripts/24gpu-3node-wandb.py"
