...
if rank == 0:
    ...
    wandb.init(
        ...
        name="Four Node Run 2.2",

        # track hyperparameters and run metadata
        config={
            "base_learning_rate": 0.001,
            "learning_rate": 0.002,  # Conservative 2x scaling for 4 nodes (vs 2 nodes)
            "batch_size_per_gpu": 24,  # Reduced from 32 to 24 for stability
            "effective_batch_size": 768,  # 24 * 32 GPUs
            "architecture": "vit_b_16",
            "epochs": 12,  # Extended for better convergence and fair comparison
            "nodes": 4,
            "total_gpus": 32,
            "scaling_strategy": "conservative_linear",
            "warmup_epochs": 1,
            }
    )
...
# Conservative learning rate scaling for 4 nodes
base_lr = 0.001
world_size = dist.get_world_size()
# Use conservative linear scaling: 2x for 4 nodes vs baseline 2 nodes
scaled_lr = base_lr * (world_size / 16) if world_size > 16 else 0.002
...
warmup_epochs = 1
total_epochs = 12
steps_per_epoch = 80  # Approximate based on dataset size
...
with HDF5Dataset("train_images.hdf5", transform=transform) as full_train_dataset:
    ...
    train_loader = DataLoader(
        train_dataset, sampler=train_sampler, batch_size=24, num_workers=7
    )

    val_sampler = DistributedSampler(val_dataset)
    val_loader = DataLoader(
        val_dataset, sampler=val_sampler, batch_size=24, num_workers=7
    )
