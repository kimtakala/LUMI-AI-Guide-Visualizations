...
if rank == 0:
    ...
    wandb.init(
        ...
        name="Eight Node Run 2.1",

        # track hyperparameters and run metadata
        config={
            "base_learning_rate": 0.001,
            "learning_rate": 0.004,  # Optimized for 8-node scaling (4x base)
            "batch_size_per_gpu": 18,  # Optimized batch size for 8 nodes
            "effective_batch_size": 1152,  # 18 * 64 GPUs
            "architecture": "vit_b_16",
            "epochs": 15,  # Optimized for 8-node convergence
            "nodes": 8,
            "total_gpus": 64,
            "scaling_strategy": "conservative_square_root",
            "warmup_epochs": 2,  # Longer warmup for 8-node stability
            }
    )
...
# Optimized learning rate scaling for 8 nodes
base_lr = 0.001
world_size = dist.get_world_size()
# Optimized scaling: 4x for 8 nodes with improved convergence
scaled_lr = 0.004
...
warmup_epochs = 2
total_epochs = 15
steps_per_epoch = 100  # Adjusted for 18 batch size and dataset size
...
with HDF5Dataset("train_images.hdf5", transform=transform) as full_train_dataset:
    ...
    train_loader = DataLoader(
        train_dataset, sampler=train_sampler, batch_size=18, num_workers=7
    )

    val_sampler = DistributedSampler(val_dataset)
    val_loader = DataLoader(
        val_dataset, sampler=val_sampler, batch_size=18, num_workers=7
    )
