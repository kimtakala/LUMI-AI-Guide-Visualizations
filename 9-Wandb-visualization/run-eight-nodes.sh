...
#SBATCH --time=10:00                 # time limit (optimized for 6 epochs)
...
# 8-node specific optimizations (64 GPUs)
export NCCL_TREE_THRESHOLD=0
export NCCL_RING_THRESHOLD=0
export NCCL_ALGO=RING
export NCCL_PROTO=Simple
export NCCL_MAX_NCHANNELS=32    # More channels for 64 GPUs
...
export NCCL_IB_TIMEOUT=90       # Shorter timeout for faster detection

# Optimized timeouts for 8-node setup (faster than 4-node)
export NCCL_BLOCKING_WAIT=1
export NCCL_ASYNC_ERROR_HANDLING=1
export NCCL_TIMEOUT=240         # 4 minutes for 64 GPUs
...
srun --cpu-bind=v,mask_cpu=$CPU_BIND_MASKS singularity exec -B ../resources/visualtransformer-env.sqsh:/user-software:image-src=/ $CONTAINER bash -c "export RANK=\$SLURM_PROCID && export LOCAL_RANK=\$SLURM_LOCALID && cd ../resources && python ../9-Wandb-visualization/scripts/wandb_ddp_visualtransformer_8nodes.py"
