...
def print_timing_summary(phase_times, total_samples, subset_size):
    ...
    print(f"{'='*60}")
    print(
        f"Subset processed: {subset_size:,} samples ({subset_size/total_samples*100:.1f}% of full dataset)"
    )
    ...
    for phase, times in phase_times.items():
        ...
        print(
            f"    Time per epoch: {full_epoch_time/60:.1f} minutes ({full_epoch_time/3600:.2f} hours)"
        )
        print(
            f"    Total training time: {full_total_time/60:.1f} minutes ({full_total_time/3600:.2f} hours)"
        )
        print()
...
def train_model_with_timing(
    ...
    print(
        f"Processing {subset_size:,} samples ({subset_size/total_samples*100:.1f}% of full {total_samples:,} samples)"
    )
...
def main():
    ...
    # Load the same HDF5 dataset as LUMI
    print("\nLoading HDF5 dataset...")
    hdf5_path = os.path.join(resources_path, "train_images.hdf5")
    ...
    with HDF5Dataset(hdf5_path, transform=transform) as full_dataset:
        ...
        print(f"Full dataset size: {total_samples:,} samples")
        print(
            f"Subset size: {subset_size:,} samples ({subset_size/total_samples*100:.1f}%)"
        )
        ...
        avg_epoch_time = np.mean(
            [
                sum(times)
                for times in zip(phase_times["training"], phase_times["validation"])
            ]
        )
        ...
        print(
            f"Speedup needed vs laptop: {full_training_time/3600:.1f}x for 1-hour training"
        )
        ...
        wandb.log(
            {
                ...
                "subset_processing_efficiency": subset_size
                / total_time,  # samples per second overall
            }
        )
